"""Tests for fools-related models: Game and GamePlayer.
...
from fools.models import Game, GamePlayer, PlayerHand, Card, CardSuit, CardRank
...
class TestGameModel:
    ...
    def test_game_str_representation(self, basic_game):
        """Test string representation shows lobby name and fools status."""
    ...
    def test_get_player_count_method(self, basic_game, test_user, second_user):
        """Test get_player_count() returns correct number of players in the game."""
...
class TestGamePlayerModel:
    ...
    def test_unique_together_constraint(self, basic_game, test_user):
        """Test that a user cannot be added to the same fools twice.

        The database enforces uniqueness on (fools, user) combination.
        """
        GamePlayer.objects.create(
            game=basic_game,
            user=test_user,
