...
This module tests Turn and Move models which track fools progression,
player actions, and move history.
"""

import pytest
from django.db import IntegrityError
from fools.models import Turn, Move, TableCard
...
class TestTurnModel:
    ...
    def test_unique_together_constraint(self, basic_game, test_user, second_user):
        """
        Tests that the (fools, turn_number) combination is unique.

        Each fools can only have one turn with a specific turn_number.
        """
        Turn.objects.create(game=basic_game, player=test_user, turn_number=1)
